...
export function createRenderer(options) {
  ...
  function patchProps(el, oldProps, newProps) {
    // 对比 props 有以下几种情况
    // 1. oldProps 有，newProps 也有，但是 val 值变更了（处理多了和变了的情况）
    // 举个栗子
    ...
    // 2. oldProps 有，而 newProps 没有了（处理少了的情况）
  ...
  //只有第一次才会 mountElement，后续都是patch更新了
  ...
  function processComponent(n1, n2, container, parentComponent) {
    // 如果 n1 没有值的话，那么就是 mount (parentComponent 也没值)
    if (!n1) {
      // 初始化 component
